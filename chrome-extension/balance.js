(function(root) {
  ...
  function formatBalanceValue(value, key = '') {
    ...
    if (typeof value === 'string') {
      ...
      if (/usd/i.test(key)) return `$${formatNumber(numeric)}`;
      if (/micros$/i.test(key)) return `$${formatNumber(numeric / 1000000)}`;
  ...
  function extractLocalApiBalance(data) {
    const candidates = [
      data?.wallet?.balance_micros,
      data?.data?.wallet?.balance_micros,
      data?.subscription?.remaining_credits_micros,
      data?.data?.subscription?.remaining_credits_micros
    ];
    for (const value of candidates) {
      const numeric = Number(value);
      if (Number.isFinite(numeric)) return formatBalanceValue(numeric, 'balance_micros');
    }
    return null;
  }
  ...
  root.extractLocalApiBalance = extractLocalApiBalance;
